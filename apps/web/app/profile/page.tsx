...
import { userService, UserProfile, ChangePasswordData } from '../../src/services/user.service';
...
export default function ProfileSettingsPage() {
  ...
  const isVipActive = user?.vipStatus === VipStatus.ACTIVE || user?.vipStatus === 'ACTIVE';
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            ...
            <p className="text-gray-500 text-sm mt-1">Your basic account details and role information</p>
          </div>
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                {isEditingUsername ? (
                  ...
                  <>
                    <span className="flex-1 text-gray-900 font-medium">{user?.username}</span>
                    <button
            ...
            <div>
              ...
              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                ...
                {user?.role === 'admin' ? 'Administrator' : isVipActive ? 'VIP Member' : 'Standard User'}
              </span>
            </div>
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl shadow-sm border border-yellow-200 mb-6 overflow-hidden">
            <div className="px-6 py-4 border-b border-yellow-200">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-yellow-800">VIP Membership</h2>
              </div>
              <p className="text-yellow-700 text-sm mt-1">Your VIP subscription details</p>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {user?.vipPlan && (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-yellow-800">Plan</span>
                  <span className="text-yellow-900 font-medium capitalize">{user.vipPlan}</span>
                </div>
              )}
              ...
              {user?.vipRequestedAt && (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-yellow-800">Registration Date</span>
                  <span className="text-yellow-900">{formatDate(user.vipRequestedAt)}</span>
                </div>
              )}
              ...
              {user?.vipExpiry && (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-yellow-800">Expiration Date</span>
                  <span className={`font-medium ${
                    new Date(user.vipExpiry) < new Date()
                      ? 'text-red-600'
                      : new Date(user.vipExpiry) < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
                      ? 'text-orange-600'
                      : 'text-yellow-900'
                  }`}>
        ...
        {!isVipActive && user?.role !== 'admin' && (
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl shadow-sm border border-indigo-200 mb-6 overflow-hidden">
            <div className="px-6 py-5">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-indigo-900">Upgrade to VIP</h3>
                  <p className="text-indigo-700 text-sm mt-1">Get access to exclusive features and premium tools</p>
                </div>
                <button
                  onClick={() => router.push('/vip-register')}
                  className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-lg hover:from-indigo-700 hover:to-purple-700 shadow-lg shadow-indigo-500/25 transition-all"
                >
                  Upgrade Now
                </button>
              </div>
          ...
          <div className="px-6 py-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">Last updated: {user?.updatedAt ? formatDate(user.updatedAt) : 'Not available'}</p>
              </div>
              <button
