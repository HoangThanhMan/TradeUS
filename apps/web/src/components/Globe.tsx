...
export default function Globe() {
  ...
  useEffect(() => {
    ...
    const defaultData = new Uint8Array([20, 70, 200]);
    const defaultTexture = new THREE.DataTexture(
      defaultData,
      1,
      1,
    ...
    const bwMaterial = new THREE.ShaderMaterial({
      ...
      vertexShader: `varying vec2 vUv; void main(){ vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 ); }`,
      fragmentShader: `uniform sampler2D baseMap; uniform float threshold; uniform float softness; uniform vec3 oceanColor; uniform vec3 landColor; varying vec2 vUv; void main(){ vec3 base = texture2D(baseMap, vUv).rgb;
        float bdom = base.b - max(base.r, base.g);
        float lum = dot(base, vec3(0.299,0.587,0.114));
        float darkBlue = (base.b > base.r) ? (0.6 - lum) : -1.0;
        float metric = max(bdom, darkBlue);
